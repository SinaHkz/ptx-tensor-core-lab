...
/*
 * Stage 08: add M-tiling via grid.y (core path, no M-tail guards).
 *
 * Delta vs stage 07:
 *   - blockIdx.y owns one 16-row M tile (`m_block_base = blockIdx.y * 16`).
 *   - A staging and C store index through that row base.
 *   - N/K behavior and guarded cp.async pipeline are unchanged.
 */
