...
// Stage-06b delta vs stage 06:
//   - Keeps the same cp.async double-buffered pipeline.
//   - Adds boundary-safe B staging + warp guards for tail N block tiles.
//   - Therefore N only needs MMA alignment (N % 8 == 0), not N % 32.
...
int main(int argc, char **argv) {
  ...
  if (K % 16 != 0) {
    std::cerr << "Stage 06b requires K to be a multiple of 16. Got K=" << K << "\n";
    return 1;
  }
  if (N % 8 != 0) {
    std::cerr << "Stage 06b requires N to be a multiple of 8. Got N=" << N << "\n";
    return 1;
  }
