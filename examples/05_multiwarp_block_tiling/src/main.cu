...
// Stage-05 delta vs stage 04:
//   - Kernel maps 4 warps per block (128 threads) onto a 16x32 C block tile.
//   - Host launch shape changes from <<<1, 32>>> to <<<1, 128>>>.
//   - This core-learning stage enforces full block-tile coverage (N % 32 == 0)
//     and defers boundary-guard handling to the follow-up 05b stage.
...
int main(int argc, char **argv) {
  ...
  // Stage-05 keeps M fixed at 16. K and N are inferred from file sizes.
  ...
  if (K % 16 != 0) {
    std::cerr << "Stage 05 requires K to be a multiple of 16. Got K=" << K << "\n";
    return 1;
  }
  if (N % BLOCK_TILE_N != 0) {
    std::cerr << "Stage 05 requires N to be a multiple of " << BLOCK_TILE_N
              << " (no boundary guards in this stage). Got N=" << N << "\n";
    return 1;
  }
  ...
  tensor_core_kernel<<<1, THREADS_PER_BLOCK>>>(dA, dB, dC, K, N);
  ...
  std::cout << "Launch: 1 block, " << WARPS_PER_BLOCK << " warps/block ("
