...
// Stage-07 delta vs stage 06b:
//   - N block-tile ownership moves from an in-kernel loop to grid.x blocks.
//   - cp.async double-buffered K-loop and boundary-safe tail handling are kept.
...
int main(int argc, char **argv) {
  ...
  if (K % 16 != 0) {
    std::cerr << "Stage 07 requires K to be a multiple of 16. Got K=" << K << "\n";
    return 1;
  }
  if (N % 8 != 0) {
    std::cerr << "Stage 07 requires N to be a multiple of 8. Got N=" << N << "\n";
    return 1;
  }
  ...
  tensor_core_kernel<<<grid_x, THREADS_PER_BLOCK>>>(dA, dB, dC, K, N);
  ...
  std::cout << "Launch: grid.x=" << grid_x
            << " block(s), block tile N=" << BLOCK_TILE_N
            << ", " << WARPS_PER_BLOCK << " warps/block ("
