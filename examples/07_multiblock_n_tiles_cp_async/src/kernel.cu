...
constexpr int BLOCK_TILE_N = WARPS_PER_BLOCK * MMA_N;  // The number of columns can be handled per block

constexpr int PIPE_STAGES = 2;
}  // namespace

/*
 * Stage 07: multi-block N-tiling with cp.async pipeline.
 *
 * Delta vs stage 06b:
 *   - N-block ownership moves from an in-kernel loop to grid.x:
 *       blockIdx.x owns one 32-column C tile.
 *   - Guarded B staging and guarded warp participation are unchanged.
 *   - Double-buffered cp.async K-loop schedule is unchanged.
 */
__global__ void tensor_core_kernel(const half *A, const half *B, float *C, int K, int N) {
    ...
    // Each block owns one N block tile. The K-loop cp.async schedule is
    // identical to stage 06b inside that ownership slice.
