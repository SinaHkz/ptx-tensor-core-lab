...
/*
 * cp.async staging glossary (stage 07 guarded pipeline helper)
 *
 ...
 *
 * Guard detail:
 * - Because one copy writes two half elements (`col` and `col+1`), the guard
 *   checks `global_col + 1 < N` before issuing cp.async.
 */
...
__device__ inline void stage_k_tile_cp_async_guarded(
    ...
    for (int b_word = block_tid; b_word < b_words_per_tile; b_word += threads_per_block) {
        ...
        if (global_col + 1 < N) {
            // Both half values covered by this 4-byte word are valid.
            cp_async_ca_shared_global_4(
                &sB_buf[b_row * block_tile_n + b_col],
                &B[(k_tile_base + b_row) * N + global_col]);
        } else {
            // Tail path: zero-fill out-of-range shared word.
