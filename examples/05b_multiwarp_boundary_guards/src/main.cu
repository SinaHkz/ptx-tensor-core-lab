...
// Stage-05b delta vs stage 05:
//   - Keeps the same 4-warp block-tiling structure and launch shape.
//   - Adds in-kernel boundary guards so partial 32-column block tiles are safe.
//   - Therefore N only needs to satisfy MMA alignment (N % 8 == 0), not N % 32.
...
int main(int argc, char **argv) {
  ...
  if (K % 16 != 0) {
    std::cerr << "Stage 05b requires K to be a multiple of 16. Got K=" << K << "\n";
    return 1;
  }
  if (N % 8 != 0) {
    std::cerr << "Stage 05b requires N to be a multiple of 8. Got N=" << N << "\n";
    return 1;
  }
