...
// Stage-06 delta vs stage 05:
//   - Kernel keeps the same 4-warp block mapping, but replaces synchronous
//     K-tile staging with a double-buffered cp.async pipeline.
//   - This core version keeps full block-tile assumption (N % 32 == 0).
...
int main(int argc, char **argv) {
  ...
  if (K % 16 != 0) {
    std::cerr << "Stage 06 requires K to be a multiple of 16. Got K=" << K << "\n";
    return 1;
  }
  if (N % BLOCK_TILE_N != 0) {
    std::cerr << "Stage 06 requires N to be a multiple of " << BLOCK_TILE_N
