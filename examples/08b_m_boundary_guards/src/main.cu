...
//   - Keeps runtime `--m` input and 2D launch.
//   - Relaxes M alignment by handling partial 16-row M tiles safely.
//   - N/K constraints are unchanged.
...
int main(int argc, char **argv) {
  ...
  if (K % 16 != 0) {
    std::cerr << "Stage 08b requires K to be a multiple of 16. Got K=" << K << "\n";
    return 1;
  }
  if (N % 8 != 0) {
    std::cerr << "Stage 08b requires N to be a multiple of 8. Got N=" << N << "\n";
    return 1;
  }
  ...
  const int grid_y = (M + MMA_M - 1) / MMA_M;
